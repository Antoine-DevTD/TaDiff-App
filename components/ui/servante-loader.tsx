export function ServanteLoader({
  label,
  description,
}: {
  label: string;
  ...
  return (
    <div className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-background px-4 text-center">
      ...
      <p className="mt-10 text-xs uppercase tracking-[0.24em] text-muted">{label}</p>
      {description ? <p className="mt-3 max-w-sm text-sm text-muted">{description}</p> : null}
    </div>
  );
}
