...
export function UnclassifiedDocuments({ documents, showId }: { documents: ShowDocument[]; showId: string }) {
  ...
  return (
    <section className="space-y-3 border-t border-border pt-6">
      <div><h3 className="font-semibold">À renseigner</h3><p className="mt-1 text-sm text-muted">TaDiff n&apos;a pas reconnu ces fichiers. Indiquez leur type pour les ranger au bon endroit.</p></div>
      {documents.map((document) => (
        <div key={document.id} className="flex flex-wrap items-center gap-3 rounded-md border border-warning/30 bg-warning/5 p-3">
          <div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{document.title}</p><p className="text-xs text-muted">Type à renseigner</p></div>
          ...
          {document.fileUrl ? <a aria-label="Télécharger" href={document.fileUrl}><Download className="h-4 w-4" /></a> : null}
