...
export default async function DashboardLayout({
  ...
  return (
    <div className="flex min-h-screen bg-background">
      ...
      {superAdmin ? null : (
        <FeedbackWidget triggerClassName="fixed bottom-7 right-24 z-40 inline-flex items-center gap-2 rounded-full border border-border bg-panel px-4 py-2.5 text-sm font-medium shadow-lg shadow-ink/10 transition hover:bg-panel-strong print:hidden" />
      )}
    </div>
  );
}
